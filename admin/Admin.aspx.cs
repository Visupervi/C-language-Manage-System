...
public partial class admin_Admin : System.Web.UI.Page
{
     ...
     protected void butt_log_Click(object sender, EventArgs e)
    {
        ...
        int bdid = 0;
        string dbpwd = "";
        sql = "Select * from admin where UserName ='" + gadm + "' and PassWord ='" + gpwd + "'";
        rds = obj.ExecReaderSql(sql);
        if (rds.Read())
        {
            Session["aid"] = rds.GetValue(rds.GetOrdinal("UserName"));//获取该用户名
            Session["aid"] = rds.GetValue(rds.GetOrdinal("PassWord"));//获取该用户密码
            Page.Response.Redirect("stgl.aspx");
            rds.Close();
            rds.Close();
        }
        else
        {
            function_all.TiaoZhuan("账号/密码错误", "Admin.aspx", 0);//提示
            Response.End();
        }
        ...
        {
            function_all.TiaoZhuan("账号/密码错误", "Admin.aspx", 0);
            Response.End();
        }
    }
}
